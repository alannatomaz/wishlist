/* ====== views (window globals) ====== */
const { Card, Stat, CatPill, PriPill, Pill } = window;

function StatsRow({ items }){
  const pending = items.filter(i=>!i.bought);
  const totalAll = pending.reduce((s,i)=>s+window.avgPrice(i),0);
  const totalBought = items.filter(i=>i.bought).reduce((s,i)=>s+window.avgPrice(i),0);
  const want = pending.filter(i=>i.pri===1).length;
  return (
    <div className="stats">
      <Stat k="Na lista" v={pending.length} />
      <Stat k="Se comprar tudo" v={window.brlk(totalAll)} accent />
      <Stat k="Já comprei" v={window.brlk(totalBought)} />
      <Stat k="Quero muito" v={want} />
    </div>
  );
}

function MetaBar({ items, meta, onSetMeta }){
  const [editing, setEditing] = React.useState(false);
  const [val, setVal] = React.useState(meta||"");
  React.useEffect(()=>{ setVal(meta||""); }, [meta]);
  const total = items.filter(i=>!i.bought).reduce((s,i)=>s+window.avgPrice(i),0);
  const has = meta>0;
      ...
      <div className="metatop">
        ...
        {editing ? (
          ...
          <button className="metaval" onClick={()=>setEditing(true)}>
            {has ? window.brl(meta) : "definir meta"}<span className="metapen">✎</span>
          </button>
        )}
      </div>
      {has && (
        <>
          ...
          <div className="metafoot">
            <span>{window.brl(total)} se comprar tudo</span>
            <span style={{ color }}>{over ? "excede em "+window.brl(total-meta) : "cabe · sobram "+window.brl(meta-total)}</span>
          </div>
        </>
      )}
    </div>
...
/* ---- toolbar: filtros + selecionar toggles + sort ---- */
function Toolbar({ filters, sidebarOn, onToggleSidebar, selOn, onToggleSel, sort, setSort }){
...
/* ---- quick chips (shown when sidebar is closed) ---- */
function QuickChips({ filters, setFilters }){
  ...
  return (
    <div className="quickchips">
      <div className="chips">
        <button className={"chip"+(!cats.length?" on":"")} onClick={()=>setFilters(f=>({...f,cats:[]}))}>Tudo</button>
        {Object.entries(window.CATS).map(([k,c])=>(
          <button key={k} className={"chip"+(cats.includes(k)?" on":"")} onClick={()=>tCat(k)}>
      ...
      <div className="chips" style={{marginTop:9}}>
        ...
        {window.PRI_ORDER.map(p=>(
          <button key={p} className={"chip"+(pris.includes(p)?" on":"")} onClick={()=>tPri(p)}>
            <span className="dot" style={{background:window.PRIOS[p].color, borderRadius:"50%"}}></span>{window.PRIOS[p].label}
          </button>
        ))}
      </div>
    </div>
...
/* ---- collapsible sidebar (richer filters) ---- */
function Sidebar({ filters, setFilters, items, onManageCats }){
  const cats = filters.cats||[], pris = filters.pris||[];
  const tCat = (c)=> setFilters(f=>({ ...f, cats: toggleIn(f.cats, c) }));
  const tPri = (p)=> setFilters(f=>({ ...f, pris: toggleIn(f.pris, p) }));
  const setStatus = (v)=> setFilters(f=>({ ...f, status:v }));
  const maxP = Math.max(100, ...items.map(i=>Math.ceil(window.avgPrice(i)/100)*100));
    ...
    <aside className="sidebar">
      ...
      <div className="side-sec">
        <div className="side-sechead">
          <h4>Categoria</h4>
          <button className="side-edit" onClick={onManageCats} title="gerenciar categorias">editar</button>
        </div>
        {Object.entries(window.CATS).map(([k,c])=>(
          <Check key={k} on={cats.includes(k)} color={c.color} onClick={()=>tCat(k)}>
      ...
      <div className="side-sec">
        <h4>Prioridade</h4>
        {window.PRI_ORDER.map(p=>(
          <Check key={p} on={pris.includes(p)} color={window.PRIOS[p].color} round onClick={()=>tPri(p)}>
            {window.PRIOS[p].label}
          </Check>
        ))}
      </div>
      ...
      <div className="side-sec">
        <h4>Status</h4>
        {[["all","Todos"],["bought","Comprados"]].map(([v,l])=>(
          <Check key={v} on={filters.status===v} onClick={()=>setStatus(v)}>{l}</Check>
        ))}
      </div>
      ...
      <div className="side-sec">
        ...
        <div className="side-rangeval">{filters.priceMax==null ? "qualquer preço" : "até "+window.brl(filters.priceMax)}</div>
      </div>
    </aside>
  );
}
...
function SelectionBar({ items, ids, onClear, onClose, onMarkBought }){
  const chosen = items.filter(i=>ids.has(i.id));
  const total = chosen.reduce((s,i)=>s+window.avgPrice(i),0);
  return (
    <div className="selbar">
      ...
      <div className="selbar-total">
        <span className="seltotal">{window.brl(total)}</span>
...
function applyFilters(items, filters, sort){
  const cats = filters.cats||[], pris = filters.pris||[];
  let out = items.filter(i=>{
    ...
    if(filters.priceMax!=null && window.avgPrice(i) > filters.priceMax) return false;
    return true;
  });
  const avg = window.avgPrice;
  const cmp = {
    "date": (a,b)=> b.date.localeCompare(a.date),
    "date-asc": (a,b)=> a.date.localeCompare(b.date),
    "price-asc": (a,b)=> avg(a)-avg(b),
    "price-desc": (a,b)=> avg(b)-avg(a),
    "pri": (a,b)=> a.pri-b.pri,
    "az": (a,b)=> a.t.localeCompare(b.t,"pt-BR"),
    "manual": ()=> 0,
  }[sort] || (()=>0);
  return out.sort(cmp);
}
...
function withH(items){ return items.map((it,i)=>({ ...it, _h: HEIGHTS[it.id % HEIGHTS.length] })); }

/* shared drag-to-reorder for Mural & List */
...
function MuralView({ items, onOpen, onAdd, sort, onReorder, sel, anyFilter }){
    ...
    <>
      ...
      <div className="masonry">
        {withH(items).map(it=> <Card key={it.id} item={it} onOpen={onOpen} drag={manual} dragProps={props(it.id)} dropover={overId===it.id}
          selecting={sel.active} selected={sel.ids.has(it.id)} onToggleSel={sel.toggle} />)}
...
function CategoryView({ items, onOpen, onAdd, sel, anyFilter }){
  if(!items.length) return <Empty onAdd={onAdd} filtered={anyFilter} />;
  const groups = {};
  ...
  return (
    <div>
      {Object.entries(window.CATS).map(([k,c])=>{
        ...
        const sub = its.filter(i=>!i.bought).reduce((s,i)=>s+window.avgPrice(i),0);
        return (
          <div className="catsec" key={k}>
            <div className="catsec-head">
              ...
              <span className="sub">{window.brlk(sub)} <span>se comprar tudo</span></span>
            </div>
            <div className="grid">
...
function ListView({ items, onOpen, onAdd, sort, onReorder, sel, anyFilter }){
  const manual = sort==="manual" && !sel.active;
  const { props, overId } = useReorder(items, onReorder, manual);
  if(!items.length) return <Empty onAdd={onAdd} filtered={anyFilter} />;
  return (
    <>
      ...
      <div className="listv">
        {items.map(it=>{
          const cat = window.catOf(it.cat);
          const avg = window.avgPrice(it);
          const cover = window.coverOf(it);
          ...
          return (
            <div key={it.id}
              ...
              {...props(it.id)}
              onClick={()=> sel.active ? sel.toggle(it.id) : onOpen(it)}>
              ...
              <div className="lt" style={{ background:`color-mix(in srgb, ${cat.color} 15%, var(--tint))`, color:cat.color }}>
                {cover ? <img src={cover.src} alt="" style={{ objectPosition: window.objPos(cover) }} /> : <span className="mono-ini">{cat.ini}</span>}
              </div>
              <div className="lmain">
                <h3>{it.t}</h3>
                <div className="lmeta">
                  <CatPill cat={it.cat} />
                  <PriPill pri={it.pri} />
                  <span style={{fontSize:10.5,color:"var(--ink3)",fontWeight:700,textTransform:"uppercase",letterSpacing:".08em"}}>{window.fmtDate(it.date)} · {it.links.length} {it.links.length===1?"loja":"lojas"}</span>
                </div>
              </div>
              <div className="lprice">
                <div className="avg">{window.brl(avg)}</div>
                <div className="rng">{window.brl(it.min)}–{window.brl(it.max)}</div>
              </div>
            </div>
          );
        })}
...
Object.assign(window, { StatsRow, MetaBar, Toolbar, QuickChips, Sidebar, SelectionBar, activeCount, applyFilters, MuralView, CategoryView, ListView, Empty });
