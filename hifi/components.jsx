/* ====== shared components (window globals) ====== */
const { CATS, PRIOS, brl, brlk, avgPrice, bestLink, fmtDate } = window;

function Thumb({ item, h }){
  const cat = window.catOf(item.cat);
  const bg = `color-mix(in srgb, ${cat.color} 15%, var(--tint))`;
  const cover = window.coverOf(item);
  const n = window.imgsOf(item).length;
  return (
    <div className="thumb" style={{ height:h, background:bg, color:cat.color }}>
      {cover
        ? <><img src={cover.src} alt={item.t} style={{ objectPosition: window.objPos(cover) }} />{n>1 && <span className="imgcount">{n} fotos</span>}</>
...
function PriceTrack({ item }){
  ...
  return (
    <div className="track">
      ...
      <div className="ends">
        <span>min {brl(item.min)}</span>
        <span>max {brl(item.max)}</span>
      </div>
    </div>
  );
}

function Card({ item, onOpen, drag, dragProps, dropover, selecting, selected, onToggleSel }){
  const avg = avgPrice(item);
...
function ImageDrop({ value, onChange, className, style, children }){
  const inputRef = React.useRef(null);
  const [over, setOver] = React.useState(false);
  const handle = (f)=>{ if(f) window.fileToImg(f, onChange); };
...
/* subtle celebration on "marquei como comprado" */
function confetti(x, y){
  const colors = ["#57314C","#7C5A6E","#7A8650","#A9833F","#9B3B54","#6F4A3A","#F1ECD9"];
  for(let i=0;i<22;i++){
    const d = document.createElement("div");
    const sz = 6+Math.random()*6;
    d.style.cssText = `position:fixed;left:${x}px;top:${y}px;width:${sz}px;height:${sz}px;
    ...
    d.animate([
      ...
      { transform:`translate(${dx}px,${dy+240}px) rotate(${Math.random()*640}deg)`, opacity:0 }
    ], { duration:900+Math.random()*500, easing:"cubic-bezier(.2,.7,.3,1)" }).onfinish = ()=>d.remove();
  }
}

/* draggable focal-point editor — drag the dot to choose what stays centred.
   Commits to the store only on release (not every move) so it stays smooth
   and reliable even with large images. */
function FocusEditor({ image, onChange, onDone }){
  const ref = React.useRef(null);
  const [pos, setPos] = React.useState({ x:image.fx, y:image.fy });
...
Object.assign(window, { Thumb, Pill, Tag, CatPill, PriPill, PriceTrack, Card, Stat, ImageDrop, FocusEditor, confetti });
