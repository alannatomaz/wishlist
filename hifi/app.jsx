/* ====== main app ====== */
const { useStore, StatsRow, MetaBar, Toolbar, QuickChips, Sidebar, SelectionBar, activeCount, applyFilters,
        MuralView, CategoryView, ListView, ItemDetail, AddEditForm, CategoryManager } = window;
const { useTweaks, TweaksPanel, TweakSection, TweakRadio, TweakColor, TweakSlider, TweakButton } = window;
...
function App(){
  ...
  const [filters, setFilters] = React.useState({ cats:[], pris:[], status:"all", priceMax:null });
  ...
  const [open, setOpen] = React.useState(null);   // item being viewed
  const [editing, setEditing] = React.useState(undefined); // item|null(new)|undefined(closed)
  const [manageCats, setManageCats] = React.useState(false);
  ...
  const openItem = (it)=> setOpen(it);
  const startAdd = ()=> setEditing(null);
  const startEdit = (it)=>{ setOpen(null); setEditing(it); };

  // keep open item fresh after edits/toggles
  ...
  const exportList = ()=>{
    ...
    a.href = url; a.download = "minha-lista-de-desejos.json"; a.click();
    ...
    <div className="app">
      <header className="topbar">
        <div className="topbar-row">
          ...
          <button className="addbtn" onClick={startAdd}><span className="p">+</span> adicionar</button>
        </div>
      </header>
      ...
      {!sidebarOn && <QuickChips filters={filters} setFilters={setFilters} />}

      {sidebarOn ? (
        <div className="withside">
          <Sidebar filters={filters} setFilters={setFilters} items={store.items} onManageCats={()=>setManageCats(true)} />
          <div className="sidemain">
            <ViewComp items={shown} onOpen={openItem} onAdd={startAdd} sort={sort} onReorder={store.reorderSubset} sel={sel} anyFilter={anyFilter} />
          </div>
        </div>
      ) : (
        <ViewComp items={shown} onOpen={openItem} onAdd={startAdd} sort={sort} onReorder={store.reorderSubset} sel={sel} anyFilter={anyFilter} />
      ...
      {liveOpen && <ItemDetail item={liveOpen} store={store} onClose={()=>setOpen(null)} onEdit={startEdit} />}
      ...
      <TweaksPanel>
        ...
        <TweakRadio label="Tema" value={t.theme} options={["Creme","Vinho"]}
          onChange={v=>setTweak("theme", v)} />
        <TweakColor label="Cor de destaque" value={t.accent}
          options={["#57314C","#7C5A6E","#7A8650","#6F4A3A","#97743A","#9B3B54"]}
          onChange={v=>setTweak("accent", v)} />
        <TweakRadio label="Estilo de destaque" value={t.highlight} options={["Sólido","Suave","Contorno"]}
          onChange={v=>setTweak("highlight", v)} />
        <TweakRadio label="Densidade" value={t.density} options={["compact","regular"]}
          onChange={v=>setTweak("density", v)} />
        <TweakSlider label="Tamanho do texto" value={t.fontSize} min={14} max={19} unit="px"
          onChange={v=>setTweak("fontSize", v)} />
        <TweakSection label="Visualização inicial" />
        <TweakRadio label="Abrir em" value={t.view} options={["mural","cat","list"]}
