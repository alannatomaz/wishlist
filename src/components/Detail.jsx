...
import { avgPrice, brl, catOf, imgsOf, objPos, fmtDate, PRIOS, CAT_PALETTE } from '../lib/utils';
import { PriceTrack, CatPill, PriPill, ImageDrop, FocusEditor, confetti } from './shared';
...
export function ItemDetail({ item, store, onClose, onEdit }){
  const avg = avgPrice(item);
  const [adjust, setAdjust] = useState(false);
  const setBest = (idx)=>store.setBest(item.id,idx);
  const markBought = (e)=>{ if(!item.bought){ const r=e.currentTarget.getBoundingClientRect(); confetti(r.left+r.width/2,r.top); } store.toggleBought(item.id); };
  const cat = catOf(item.cat, store.cats);
      ...
      <div className="detail-grid">
          ...
          {cover ? (
            adjust ? (
              ...
              <div className="bigthumb" style={{ background:`color-mix(in srgb, ${cat.color} 18%, var(--tint))` }}>
                <img className="dropimg" src={cover.src} alt={item.t} style={{ objectPosition:objPos(cover) }} />
              </div>
            )
          ) : (
        ...
        <div className="detail-info">
          <div className="pillrow" style={{display:"flex",gap:7,flexWrap:"wrap"}}>
            <CatPill cat={item.cat} solid cats={store.cats} />
          ...
          <div className="detail-date">📅 adicionado em {fmtDate(item.date)}</div>
          ...
          {item.notes&&<><div className="section-label" style={{marginTop:22}}>Minhas notas</div><div className="notebox">{item.notes}</div></>}
          <div className="detail-actions">
            <button className={"act primary"+(item.bought?" undo":"")} onClick={markBought}>{item.bought?"desmarcar compra":"marquei como comprado"}</button>
...
export function AddEditForm({ item, store, onClose }){
  const editing = !!item;
  const [f, setF] = useState(()=> item ? {
    ...item, imgs:imgsOf(item), links:item.links.map(l=>({...l}))
  } : {
    t:"", cat:Object.keys(store.cats)[0]||"eletro", pri:2, min:"", max:"", notes:"", bought:false, imgs:[],
      ...
      <div className="form">
        ...
        <label className="fld"><span>Categoria</span>
          <select className="inp" value={f.cat} onChange={e=>upd("cat",e.target.value)}>
            {Object.entries(store.cats).map(([k,c])=><option key={k} value={k}>{c.name}</option>)}
          </select>
        </label>
        <label className="fld"><span>Prioridade</span>
          <select className="inp" value={f.pri} onChange={e=>upd("pri",e.target.value)}>
            {Object.entries(PRIOS).map(([k,p])=><option key={k} value={k}>{p.label}</option>)}
          </select>
        </label>
          ...
          <div className="linkedit">
            {f.links.map((l,i)=>(
              <div className="linkedit-row" key={i}>
                <button type="button" className={"starbtn"+(l.best?" on":"")} onClick={()=>setBest(i)}>★</button>
                ...
                <button type="button" className="rmlink" onClick={()=>rmLink(i)}>✕</button>
              </div>
            ))}
...
export function CategoryManager({ store, onClose }){
  const [rows, setRows] = useState(()=>Object.entries(store.cats).map(([key,c])=>({ key, name:c.name, color:c.color })));
      ...
      <div className="cm-list">
        {rows.map((r,i)=>(
          <div className="cm-row" key={r.key}>
            ...
            <button type="button" className="rmlink" onClick={()=>del(i)}>✕</button>
          </div>
        ))}
